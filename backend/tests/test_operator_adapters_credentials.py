...
async def test_explorer_open_folder_verifies(tmp_path):
    """Â§44: open folder com verificaÃ§Ã£o de janela REAL.

    A pasta vive em .test-temp/ (basetemp do projeto, ACL do usuÃ¡rio atual).
    ORDEM OBRIGATÃ“RIA: abrir â†’ verificar janela visÃ­vel â†’ sÃ³ entÃ£o permitir
    qualquer remoÃ§Ã£o. Nada Ã© apagado antes da verificaÃ§Ã£o terminar e a raiz
    .test-temp nunca Ã© removida pelo teste.
    """
    ...
    # VerificaÃ§Ã£o INDEPENDENTE da janela antes de qualquer cleanup:
    ...
    assert window_seen, "Janela do Explorer nÃ£o confirmada; nada foi removido."

    # Fechamento GRACIOSO (WM_CLOSE) apenas da janela que este teste abriu â€”
    ...
    # Cleanup tardio e controlado: apenas o CONTEÃšDO desta subpasta numerada,
...
async def test_terminal_unsupported_action_is_honest(tmp_path):
    """Â§45: execuÃ§Ã£o de comando real permanece no system_shell."""
...
def test_credential_create_requires_explicit_interaction(tmp_path, monkeypatch):
    ...
    assert result["error_code"] == "APPROVAL_REQUIRED"  # Â§90
...
def test_credential_roundtrip_and_metadata_only(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(creds_mod, "_VAULT_FILE", tmp_path / "vault.bin")
    secret = "fake-token-not-a-real-credential-1234"
    broker = _broker(tmp_path)
    ...
    # Â§87: metadata-only; segredo nunca aparece.
    ...
    # Â§89: resolve() existe para uso INTERNO e retorna o valor.
    assert broker.resolve("home_assistant") == secret

    # Â§93/Â§94: injection scoped por processo/header.
    ...
    # Â§96: guard explÃ­cito de vazamento.
    creds_mod.assert_no_leak(listing, secret)  # passa

    rotated = broker.rotate("home_assistant", "new-secret-456")  # approvals=None -> APPROVAL_REQUIRED? NÃ£o: rotate exige approval via gate; gate None => APPROVAL_REQUIRED
...
def test_credential_delete_without_gate_fails_closed(tmp_path, monkeypatch):
    ...
    assert deleted["error_code"] == "APPROVAL_REQUIRED"  # Â§91
...
def test_dpapi_fallback_store_roundtrip(tmp_path, monkeypatch):
    ...
    # O store EM DISCO Ã© opaco (DPAPI), nunca plaintext.
...
def test_cross_context_rejection_is_mandatory():
    ...
    with pytest.raises(CrossContextRejectionError):
        registry.get(task.context_id, expected_kind=ContextKind.JOB)  # Â§250

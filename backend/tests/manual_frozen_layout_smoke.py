...
Simula o modo instalado (NYRA_FROZEN=1 + NYRA_DATA_HOME temporário) e valida:
...
SANDBOX = Path(sys.argv[1]) if len(sys.argv) > 1 else REPO.parent / ".tmp" / "nyra-frozen-smoke"


def main() -> int:
    os.environ["NYRA_FROZEN"] = "1"
    os.environ["NYRA_DATA_HOME"] = str(SANDBOX)
    os.environ["NYRA_WATCHDOG_ENABLED"] = "0"
    ...
    assert settings.database_path == SANDBOX / "data" / "nyra.db", settings.database_path

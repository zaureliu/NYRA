...
export function PronunciationLab({ speaker }: { speaker: string }) {
  ...
  const play = async (speech: string) => { const response = await fetch('/api/voice/synthesize', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ provider, voice: provider === 'edge_tts' ? 'pt-BR-ThalitaMultilingualNeural' : provider === 'kokoro' ? 'kazumi_voice_v2' : 'default', text: speech, state: 'neutral', edge_rate: '-5%', edge_pitch: '+0Hz', edge_volume: '+0%' }) }); const value = await response.json(); if (!response.ok) { setNotice(value.detail ?? 'Provider indisponível'); return }; const sourceUrl = await backendObjectUrl(value.audio_url); const audio = new Audio(sourceUrl); const release = () => releaseBackendObjectUrl(sourceUrl); audio.addEventListener('ended', release, { once: true }); audio.addEventListener('error', release, { once: true }); if (speaker !== 'default' && 'setSinkId' in audio) await (audio as HTMLAudioElement & {setSinkId(id:string):Promise<void>}).setSinkId(speaker); await audio.play() }

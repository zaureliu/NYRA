...
O executável fica em `desktop/src-tauri/target/release/kazumi-desktop.exe` e pode
ser iniciado com `.\start-kazumi.ps1`. Modelos TTS são lidos do runtime
`KAZUMI_DATA_HOME`/`%LOCALAPPDATA%\KAZUMI`; nenhum banco, segredo, log ou estado do
operador é incorporado ao executável.

No host de desenvolvimento, `start-kazumi.ps1`/`KAZUMI.lnk` iniciam o backend pela
...
- Segure “Falar com KAZUMI”, fale e solte. O navegador solicitará acesso ao microfone.

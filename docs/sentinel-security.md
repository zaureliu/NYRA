...
- read-only do ponto de vista da NYRA;
...
Rotação: gere outro token no Sentinel, atualize os dois `.env`/secret store e reinicie o Sentinel. A NYRA perde autenticação de forma visível e não recebe detalhes protegidos até o token correto ser salvo.

...
Perfis disponíveis: `natural`, `focused`, `concerned`, `amused` e `alert`. A `NYRA Digital Signature` é opcional e quase imperceptível; não há pitch/formant extremo, autotune, bitcrusher ou vocoder.

Em `Settings > Voice Lab > Voice Processor`, o usuário controla Enabled, preset, EQ, compression, presence, output gain e signature, e gera A/B do mesmo áudio base em `Original TTS` e `NYRA Processed`.

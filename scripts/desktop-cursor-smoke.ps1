...
if (-not $OutputDirectory) {
    $OutputDirectory = Join-Path (Initialize-KazumiRuntimePaths).Temp 'desktop-cursor-smoke'
}
Add-Type -AssemblyName System.Windows.Forms
Add-Type @'
using System;
using System.Runtime.InteropServices;
public static class KazumiCursorSmokeNative {
...
$original = New-Object KazumiCursorSmokeNative+Point
[KazumiCursorSmokeNative]::GetCursorPos([ref]$original) | Out-Null
$captures = @()
...
try {
    ...
    foreach ($screen in $screens) {
        ...
        foreach ($entry in $points.GetEnumerator()) {
            [KazumiCursorSmokeNative]::SetCursorPos($entry.Value[0], $entry.Value[1]) | Out-Null
    ...
    [KazumiCursorSmokeNative]::SetCursorPos($original.X, $original.Y) | Out-Null
}
